...
for item in "$source_directory"/*; do
    if [ -f "$item" ] || [ -d "$item" ]; then
        ...
        # Extract the TV show name and season number from the file or folder name
        filename=$(basename "$item")
        tv_show_name=$(echo "$filename" | sed -E 's/(.*[.])?[Ss]([0-9]+)[Ee]([0-9]+).*/\1/' | tr '.' ' ')
        season_number=$(echo "$filename" | sed -E 's/(.*[.])?[Ss]([0-9]+)[Ee]([0-9]+).*/\2/')
        ...
        if [ -z "$show_folder" ]; then
            ...
            show_folder=${show_folder//./ }  # Replace dots with spaces
            mkdir "$show_folder"
